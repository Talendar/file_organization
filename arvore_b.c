...
static void imprimir_pagina(BTPagina *p);
...
/**
 * Cria um arquivo de índice árvore-B a partir de um arquivo de dados já existente.
 * 
 * @param dados arquivo de dados.
 * @param bt nome do arquivo de índice.
 * @return true caso a operação tenha ocorrido com sucesso; false no caso contrário.
 */
...
/**
 * Insere uma nova chave de busca no índice árvore-B. Para diminuir a quantidade de escritas em disco necessárias em casos de múltiplas inserções seguidas, fica a cargo do chamador escrever o cabeçalho atualizado do índice.
 * 
 * @param chave chave de busca a ser indexada.
 * @param ponteiro campo de referência para o registro que está sendo indexado.
 * @param cab cabeçalho do arquivo de índice árvore-B.
 * @param bt arquivo binário que armazena o índice árvore-B.
 */
void bt_inserir(int chave, int ponteiro, BTCabecalho *cab, FILE *bt)
{   
    BTItem item = { .chave = chave, .ponteiro = ponteiro};
...
/**
 * Atualiza o status do arquivo em seu cabeçalho.
 * 
 * @param status novo status.
 * @param bt binário do arquivo de índice.
 */
void bt_escrever_status(char status, FILE *bt) {
    fseek(bt, 0, SEEK_SET);                // move o ponteiro de escrita para o início do arquivo
    fwrite(&status, 1, 1, bt);             // escreve o status
}
 ...
 * @param filho_dir o filho direito do item.
 * @param p a página onde ocorrerá a inserção.
 */
static void bt_inserir_na_pagina(BTItem *item, int filho_dir, BTPagina *p)
{
...
/**
 * Realiza uma operação de split na árvore-B.
 * 
 * @param item item a ser inserido.
...
void bt_escrever_pagina(BTPagina *p, int rrn, FILE *bt) 
{
    ...
    /* Preenchendo com NIL (filhos inexistentes) */
    int q = MAX_CHAVES - p->n;             // quantidade de filhos inexistentes
    const int nil = NIL;
    for(int i = 0; i < q; i++)
        fwrite(&nil, 4, 1, bt);
}
...
/**
 * Realiza uma operação de busca na árvore-B a partir da chave fornecida.
 * 
 * @param resultado RRN obtido pela busca.
 * @param chave valor da chave.
 * @param bt binário do arquivo de índices árvore-B.
 * @return número de páginas acessadas durante a busca.
 */
...
/**
 * Função auxiliar de busca recursiva na árvore-B.
 * 
...
/**
 * Cria (sem escrever em disco) uma nova página da árvore-B. 
 * 
 * @return ponteiro para a página criada, em caso de sucesso, ou NULL, em caso de falha de alocação de memória.
 */
...
/**
 * Cria um item da árvore-B.
 * 
 * @param chave chave do item.
 * @param ponteiro ponteiro para o próximo nó da árvore-B.
 * @return ponteiro para o item criado, em caso de sucesso, ou NULL em caso de falha de alocação de memória.
 */
...
/**
 * Cria e escreve um novo nó/página raiz da árvore-B.
 * 
 * @param item primeiro item a ser armazenado na raiz.
 * @param filho_dir filho direito do item.
 * @param filho_esq filho esquerdo do item.
 * @param bt binário do arquivo de índice árvore-B.
 * @param cab cabeçalho do arquivo de índice árvore-B.
 */
...
/**
 * Retorna true caso o arquivo de índice não possua nenhuma chave indexada e false no caso contrário.
 */
bool bt_indice_vazio(FILE *bt) {
    fseek(bt, 5, SEEK_SET);
    int nroChaves;  fread(&nroChaves, 4, 1, bt);
    return nroChaves == 0;
}
...
/**
 * Imprime as informações de uma página. Função usada para testes.
 */
static void imprimir_pagina(BTPagina *p) {
    printf("n = %d  |  nivel = %d  |  itens: ", p->n, p->nivel);
    for(int i = 0; i < MAX_CHAVES; i++) 
        printf("{%d, %d} ", p->itens[i].chave, p->itens[i].ponteiro);
